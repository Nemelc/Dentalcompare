(async () => {
  ...
  const extractProduct = (html, url) => {
    ...
    if (mpnMatch) {
      const v = clean(mpnMatch[1]).split(/\b(?:En stock|Disponible|Rupture|Sur commande)\b/i)[0].trim();
    ...
    const crumbs = crumbsRaw.filter((x, i) => i === 0 || x !== crumbsRaw[i - 1]);
  ...
  console.clear();
  console.log('DentalCompare — test automatique Mega Dental');
  console.log('Chargement des sitemaps…');

  const allUrls = [];
  for (const sitemapUrl of SITEMAPS) {
    const res = await fetch(sitemapUrl, { credentials: 'include', cache: 'no-store' });
    if (!res.ok) throw new Error(`Sitemap inaccessible: ${res.status} ${sitemapUrl}`);
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    try {
      ...
      if (res.status === 403 || res.status === 429 || challengeMarkers.some(m => low.includes(m))) {
        console.warn(`Protection détectée (${res.status || 'challenge'}). Arrêt sans contournement.`);
      ...
      if (!res.ok) {
        ...
        if (!product.name || product.price_eur == null) {
          ...
          errors.push({ url, status: res.status, error: 'incomplete_product', product });
        } else {
  ...
  const payload = {
    source: 'mega_dental_bulk_browser_test_v1',
