...
(async () => {
  ...
  const CFG = {
    ...
    storageKey: 'dentalcompare_gacd_capture_v4'
  };
  ...
  const state = Object.assign({version:4, productUrls:[], products:{}, errors:[], categoryPagesDone:[], startedAt:new Date().toISOString()}, loadState());
  ...
  function createFrame(){
    const old = document.getElementById('dc-gacd-capture-frame');
    if (old) old.remove();
    ...
    return new Promise((resolve, reject) => {
      ...
      frame.onload = async () => {
        try {
          ...
          const title = clean(doc.title || '');
          const txt = clean(doc.body?.innerText || '').slice(0,2500);
          if (/just a moment|attention required|access denied|verify you are human|security check/i.test(title+' '+txt)) {
            return finish(reject, new Error('PROTECTION_CHALLENGE'));
          }
          // Le catalogue GACD est injecté côté navigateur : attendre les cartes .result.
          const started = Date.now();
          while (Date.now() - started < CFG.renderWaitMs - 1000) {
            const cards = doc.querySelectorAll('a.result[href]');
            if (cards.length) return finish(resolve, doc);
            await sleep(250);
          }
          // Une catégorie vide peut légitimement ne contenir aucune carte.
          return finish(resolve, doc);
        } catch (e) {
          finish(reject, e);
        }
      };
  ...
  function productLinksFromRendered(doc){
  ...
  function nextPageFromRendered(doc, currentUrl){
    const current = new URL(currentUrl);
    const candidates = [
      ...doc.querySelectorAll('a[rel="next"][href], a.next[href], .pages-item-next a[href], .pagination a[href], .pages a[href]')
    ].map(a => abs(a.getAttribute('href'))).filter(Boolean);
    for (const u of candidates) {
      try {
        const x = new URL(u);
        if (x.origin !== location.origin) continue;
        if (x.pathname !== current.pathname) continue;
        if (x.href === current.href) continue;
        if (/(?:[?&](?:p|page)=\d+)/i.test(x.search)) return x.href;
      } catch {}
    }
    return '';
  }
  ...
  function parseProduct(doc, url){
    ...
    const variants=[];
    const rows = doc.querySelectorAll('tr, [class*="variant"], [class*="declinaison"], [class*="reference"]');
  ...
  console.log('[GACD v4] Découverte des catégories dentaires…');
  // Le menu chargé dans la page courante contient déjà les sous-catégories.
  const seeds = categorySeeds(document);
  console.log('[GACD v4] Sous-catégories retenues:', seeds.length);

  const frame = createFrame();
  const known = new Set(state.productUrls);

  for(let si=0; si<seeds.length; si++){
    let pageUrl = seeds[si];
    const seen = new Set();
    let pageIndex = 0;
    ...
    while(pageUrl && pageIndex < CFG.maxPagesPerCategory && !seen.has(pageUrl)){
      seen.add(pageUrl);
      pageIndex++;
      if(state.categoryPagesDone.includes(pageUrl)) {
        pageUrl = '';
        continue;
      }
      try{
        const doc = await loadRendered(frame, pageUrl);
        const links = productLinksFromRendered(doc);
        let added = 0;
        links.forEach(u=>{ if(!known.has(u)){ known.add(u); state.productUrls.push(u); added++; } });
        const next = nextPageFromRendered(doc, pageUrl);
        state.categoryPagesDone.push(pageUrl);
        saveState(state);
        console.log(`[GACD v4] catégorie ${si+1}/${seeds.length} page ${pageIndex}: ${links.length} produits, +${added}, total ${known.size}${next ? ', page suivante oui' : ', fin catégorie'}`);
        pageUrl = next;
        await sleep(CFG.delayMs);
      }catch(e){
        console.warn('[GACD v4]',pageUrl,e.message);
        state.errors.push({url:pageUrl,error:e.message,at:new Date().toISOString()});
        saveState(state);
        if(/^PROTECTION_/.test(e.message)) { download('gacd_partial_'+stamp()+'.json',state); frame.remove(); throw e; }
        break;
      }
    }
  }
  ...
  frame.remove();
  console.log('[GACD v4] Enrichissement des fiches:',state.productUrls.length);
  ...
  for(const url of state.productUrls){
    ...
    try{
      ...
      state.products[url]=rows;
      done++; session++;
      saveState(state);
      console.log(`[GACD v4] ${done}/${state.productUrls.length} — ${rows.length} référence(s) — ${url}`);
      if(session % CFG.pauseEvery===0){ console.log('[GACD v4] Pause 45 s…'); await sleep(CFG.pauseMs); }
      await sleep(CFG.delayMs);
    }catch(e){
      console.warn('[GACD v4]',url,e.message);
  ...
  const output={source:'gacd_browser_public_capture_v4',captured_at:new Date().toISOString(),product_pages:state.productUrls.length,total_products:products.length,errors:state.errors,products};
  download('gacd_catalog_'+stamp()+'.json',output);
  console.log('[GACD v4] TERMINÉ:',products.length,'références. Fichier téléchargé.');
})();