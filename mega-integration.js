...
(function(){
  ...
  let products=[],filtered=[],category='all',query='',sort='default',visible=PAGE_SIZE;
  ...
  function normalize(raw,merchant){
    ...
    return {merchant:raw.merchant||merchant,name:String(raw.name||'').trim(),brand:String(raw.brand||'').trim(),category:String(raw.dental_category||raw.category||'Autres').trim(),variant:raw.variant||'',packaging:raw.packaging||'',price,image:raw.image_url||raw.image||'',availability:raw.availability||'',merchantReference:raw.merchant_reference||'',manufacturerReference:raw.manufacturer_reference||'',ean:raw.ean||'',url:raw.source_url||raw.url||''};
  }
  const usable=p=>p.name&&Number.isFinite(p.price)&&p.price>0&&p.image&&p.url;
  function makeGroup(o,id){return {id,name:o.name,brand:o.brand,category:o.category,image:o.image,manufacturerReference:o.manufacturerReference,ean:o.ean,offers:[o]};}
  ...
  function apply(){let list=category==='all'?products:products.filter(p=>p.category===category);if(query)list=list.filter(p=>haystack(p).includes(query.toLowerCase()));list=[...list];if(sort==='price-asc')list.sort((a,b)=>a.minPrice-b.minPrice);filtered=list;const title=document.getElementById('page-title'),sub=document.getElementById('page-sub');if(title)title.textContent=query?`Résultats pour « ${query} »`:(category==='all'?'Catalogue dentaire':category);if(sub){const offers=filtered.reduce((n,p)=>n+p.offers.length,0);sub.textContent=`${filtered.length.toLocaleString('fr-FR')} produit(s) · ${offers.toLocaleString('fr-FR')} offre(s) comparées`;}draw();}
